...
if (nodeEnv !== 'test') {
    app.listen(PORT, () => {
        ...
        console.log('Google Client ID loaded:', !!require('./config').googleClientId ? 'Yes' : 'No - WARNING (Google Auth will fail)!');
        console.log('Facebook Client ID loaded:', !!require('./config').facebookClientId ? 'Yes' : 'No - WARNING (Facebook Auth will fail)!');
    });
} else {
